...
```bash
./infra/checks/run-backend-tests.sh
```

Docker-level MVP smoke:

```bash
./infra/checks/run-mvp-smoke.sh
```

Run all checks:

```bash
./infra/checks/run-all-checks.sh
```

Frontend UI automation:

```bash
./infra/checks/run-ui-e2e.sh
```

Prepare a human validation session:
...
The downloader under `apps/downloader/` is intentionally separate from the web UI.

```bash
./apps/downloader/run.sh BV1zV2QBtE39
./apps/downloader/run.sh BV1zV2QBtE39 --quality 64
```
...
BiliFocus is already past the stage of being "just a demo".

It now has the beginnings of a real workflow product: live search, local persistence, structured cleanup, local credential maintenance, and room for better experience design. A good README should not only explain how to run it. It should also make clear why this product is worth continuing to shape.
